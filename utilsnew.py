...
def train_exp(model, criterion, optimizer, X_train, A_train, y_train, method, mode, lam, batch_size=500, niter=100,alpha=0.1,beta=0.35,baselinelam=0.5,relax_loss=None):
    model.train()
    ...
    for it in range(niter):
        ...
        ## softlabel before
        y_hard=copy.deepcopy(batch_y)  
        if 'labelsoft' in method:
            delta,y_pre=regloss(model_pre, batch_x_0, batch_x_1,mode=mode,batch_x=batch_x)
            ...
            # batch_y=add_reg_y(lam,delta,batch_y_soft,sensitive_attribute)
            #TODO:back
            if method=='labelsoft_addreg':
                batch_y=soften_batch_y(batch_y,y_pre,alpha)
                loss_reg,_=regloss(model, batch_x_0, batch_x_1,mode=mode)
                loss_reg=torch.abs(loss_reg)
            elif method=='labelsoft_noreg':# label smooth indeed
                batch_y=soften_batch_y(batch_y,delta,lam)# no alpha, i.e. alpha=lam
            elif method =='labelsoft_pure':#TODO: multi-label onehot flatten
                batch_y_soft=soften_batch_y(batch_y,y_pre,alpha)
                loss_reg,_=regloss(model, batch_x_0, batch_x_1,mode=mode)
                batch_y=add_reg_y(lam,loss_reg,batch_y_soft,sensitive_attribute)# a soften label
            else:
                print('not an available method!!!')
                os._exit(0)
        elif method == 'erm':
            pass
        elif method == 'reg':
        ...
        # final loss
        if method == 'fmu' or method == 'reg' or method=='labelsoft_addreg':
            loss = loss_sup + baselinelam*loss_reg
            # print(loss_reg)
            if method== 'labelsoft_addreg':
                loss = (loss - beta).abs()
        elif method=='labelsoft_pure' and relax_loss!=None:
            # # TODO: current implementation has bugs
            # binary_output=torch.tensor(binary_value(output)).cuda().float()
            # correct = torch.eq(binary_output, y_hard).float()
            # loss_full = relax_loss(output, y_hard)
            # # # need one hot for multi-classes
            # # output = torch.cat((output, 1 - output), dim=1)
            # # batch_y = torch.cat((batch_y, 1 - batch_y), dim=1)
            # loss_soften = relax_loss(output, batch_y)
            # loss = (1 - correct) * loss_soften - 1. * loss_full
            # loss = loss.mean()
            # loss = (loss_sup - beta).abs()# gradient ascent
            loss=loss_sup#TODO:back
        else:
            loss = loss_sup
...
def visualize_results(result_path_list,method,mode,misc='',size=None,save_dir=None):
    ...
    for path in result_path_list:
        ...
        method,mode,lam=base_name.split('-')
